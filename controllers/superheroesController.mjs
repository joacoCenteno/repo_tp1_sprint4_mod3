...
export async function obtenerTodosLosSuperheroesController(req, res){
    try{
        ...
        res.render('dashboard', { superheroesFormateados, msg: req.query.msg, title: 'Listado de Superheroes' });
    }catch(err){
        console.log(err.msg);
        res.status(500).redirect('/api/heroes?msg=Error al obtener los heroes :(');
    }
}
    ...
    try{
       ...
       // const nuevo_superheroe = req.body;
        ...
        if(!heroe_actualizado){
            return res.status(404).redirect('api/heroes?msg=Error al actualizar el heroe :(');
        }
...
export function nuevoHeroeFormController(req,res){
    res.render('addSuperhero', {errors:[], hero:{}, title: 'Formulario Nuevo Heroe'});
}

export async function editarHeroeFormController(req,res){
    const {id} = req.params;
    const heroe =  await obtenerSuperheroePorId(id);
    
    res.render('editSuperhero',{errors:[], hero:heroe, title: 'Formulario Editar Heroe'});
}